...
async function loadSettings() {
    return new Promise<void>((resolve) => {
        chrome.storage.sync.get(STORAGE_KEY, (res) => {
            const s = res?.[STORAGE_KEY]?.tiktok;
...
chrome.storage.onChanged.addListener((changes) => {
    if (changes[STORAGE_KEY]) {
        const s = changes[STORAGE_KEY].newValue?.tiktok;
...
function handleVisible(video: HTMLVideoElement) {
    if (!synchedScrollrotSettings.enabled) {
        console.log("[ScrollRot] disabled or not tiktok page; no pausing");
        return;
    }
...
(async function init() {
    await loadSettings();
    console.log('[ScrollRot] tiktok content script ready');
...
// src/content/tiktokContent.ts
(() => {
    console.log('[ScrollRot] tiktok content script loaded');
    const banner = document.createElement('div');
    banner.textContent = 'ScrollRot: tiktok script active';
