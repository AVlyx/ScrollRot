...
class VideoBlocker {
  ...
  private async init(): Promise<void> {
    ...
    this.reelsWatchedCount = await getNumberWatchedShortVids("facebook");
  }
  ...
  private findReelContainers(): HTMLElement[] {
    ...
    // Facebook Reels selectors - try multiple approaches
    ...
    videos.forEach((video) => {
      ...
      // Facebook uses various structures, so we try multiple selectors
      let container = video.closest("div[role='article']");
    ...
    // Method 2: If no containers found, look for Facebook's specific structure
    if (containers.length === 0) {
      // Facebook Reels often use specific class patterns or data attributes
      const reelCandidates = document.querySelectorAll('div[role="main"] div');
  ...
  private updateNumberWatchedReels() {
    ...
    setNumberWatchedShortVids("facebook", this.reelsWatchedCount);

    if (this.reelsWatchedCount >= this.config.maxReelCount) {
      setReelLimitReachedDOM("facebook");
    }
  }
  ...
  private blockVideo(video: HTMLVideoElement, container: HTMLElement): void {
    ...
    // Use capture phase to intercept before Facebook's handlers
  ...
  private addBlockIndicator(container: HTMLElement, unblockTime: number): void {
    ...
    const existingOverlay = videoWrapper.querySelector(".facebook-reels-blocker-overlay");
    if (existingOverlay) {
      return;
    }
    ...
    overlay.className = "facebook-reels-blocker-overlay";
  ...
  `;
    ...
    countdownText.className = "facebook-reels-blocker-countdown";
  ...
  `;
    ...
    counterText.className = "facebook-reels-counter-text";
