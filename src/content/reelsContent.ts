...
async function loadSettings() {
    return new Promise<void>((resolve) => {
        chrome.storage.sync.get(STORAGE_KEY, (res) => {
            const s = res?.[STORAGE_KEY]?.instagram;
...
chrome.storage.onChanged.addListener((changes) => {
    if (changes[STORAGE_KEY]) {
        const s = changes[STORAGE_KEY].newValue?.instagram;
...
function handleVisible(video: HTMLVideoElement) {
    if (!synchedScrollrotSettings.enabled) {
        console.log("[ScrollRot] disabled or not instagram page; no pausing");
        return;
    }
...
(async function init() {
    await loadSettings();
    console.log('[ScrollRot] instagram content script ready');
...
// src/content/instagramContent.ts
(() => {
    console.log('[ScrollRot] instagram content script loaded');
    const banner = document.createElement('div');
    banner.textContent = 'ScrollRot: instagram script active';
